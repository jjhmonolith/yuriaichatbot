...
exports.SystemPromptController = void 0;
const models_1 = require("../models");
const SystemPromptVersion_1 = require("../models/SystemPromptVersion");
class SystemPromptController {
    // 모든 시스템 프롬프트 조회
    static getAllPrompts(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const prompts = yield models_1.SystemPrompt.find()
                    .sort({ key: 1 });
                res.json({
                    success: true,
                    data: {
    ...
    // 특정 프롬프트 조회
    static getPromptByKey(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { key } = req.params;
                const prompt = yield models_1.SystemPrompt.findOne({ key, isActive: true });
                if (!prompt) {
    ...
    static createPrompt(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                // 키 중복 확인
                const existingPrompt = yield models_1.SystemPrompt.findOne({ key });
                if (existingPrompt) {
                ...
                const prompt = new models_1.SystemPrompt({
                    key,
                    name,
                    description,
                    content,
    ...
    static updatePrompt(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const prompt = yield models_1.SystemPrompt.findById(id);
                if (!prompt) {
                    return res.status(404).json({
                        success: false,
                        message: 'System prompt not found'
                ...
                // 현재 버전을 히스토리에 저장
                const currentVersion = new SystemPromptVersion_1.SystemPromptVersion({
                ...
                const allVersions = yield SystemPromptVersion_1.SystemPromptVersion.find({ promptKey: prompt.key })
                    .sort({ version: -1 });
                if (allVersions.length > 10) {
                    const versionsToDelete = allVersions.slice(10);
                    yield SystemPromptVersion_1.SystemPromptVersion.deleteMany({
    ...
    static deletePrompt(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                yield models_1.SystemPrompt.findByIdAndDelete(id);
                res.json({
                    success: true,
    ...
    // 특정 프롬프트 초기화
    static initializePrompt(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { key } = req.params;
...
해설은 논리적이고 체계적으로 작성하여 학습자의 이해를 돕고, 유사한 문제에 응용할 수 있는 능력을 기를 수 있도록 해주세요.`
                ...
                if (existingPrompt) {
                    ...
                    yield existingPrompt.save();
                    prompt = existingPrompt;
                }
                else {
                    // 새 프롬프트 생성
                    prompt = new models_1.SystemPrompt(promptData);
                    yield prompt.save();
                }
                res.json({
                    success: true,
    ...
    static getPromptVersions(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { key } = req.params;
                // 현재 활성 프롬프트 확인
                const currentPrompt = yield models_1.SystemPrompt.findOne({ key });
                if (!currentPrompt) {
                    return res.status(404).json({
                        success: false,
                        message: 'System prompt not found'
                ...
                // 최근 10개 버전 조회
                const versions = yield SystemPromptVersion_1.SystemPromptVersion.find({ promptKey: key })
                    .sort({ version: -1 })
                    .limit(10);
                res.json({
                    success: true,
    ...
    // 특정 버전으로 되돌리기
    static revertToVersion(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { key } = req.params;
                ...
                // 되돌릴 버전 찾기
                const targetVersion = yield SystemPromptVersion_1.SystemPromptVersion.findOne({
                    promptKey: key,
                    version: version
                });
                if (!targetVersion) {
                ...
                // 현재 버전을 히스토리에 저장
                const currentBackup = new SystemPromptVersion_1.SystemPromptVersion({
    ...
    static initializeDefaultPrompts(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
...
해설은 논리적이고 체계적으로 작성하여 학습자의 이해를 돕고, 유사한 문제에 응용할 수 있는 능력을 기를 수 있도록 해주세요.`
                            ...
                            const promptData = defaultPrompts[key];
                            if (promptData) {
                                const prompt = new models_1.SystemPrompt(promptData);
                                yield prompt.save();
                                results.push(prompt);
                            }
                        }
...
exports.SystemPromptController = SystemPromptController;
